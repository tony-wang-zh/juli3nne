...
def upload():
    ...
    # the configs are a list of JSON strings
    configs_raw = request.form.getlist('configs')
    configs = [json.loads(c) for c in configs_raw]
    
    # 3. Access Z Offset
    z_offset = request.form.get('z_offset')

    # Example processing
    print(f"Received {len(stl_files)} files.")
    for i, file in enumerate(stl_files):
        print(f"File {i+1}: {file.filename} | Config: {configs[i]}")
